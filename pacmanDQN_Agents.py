...
class PacmanDQN(game.Agent):
    def __init__(self, args):
        ...
        self.params['shield'] = args['shield']
    ...
    def observation_step(self, state):
        if self.last_move is not None:
            ...
            if reward > 20:
                ...
                self.last_reward = -10.  # Punish time (Pff..)
            ...
            experience = (self.last_state, float(self.last_reward), self.get_value(self.last_move), self.current_state, self.terminal)
    ...
    def getAction(self, state):
        ...
        move_copy = move
        if self.params['shield']:
            safe = False
            i = 0

            while not safe and len(legal) != 0:
                ...
                for j in range(num_ghosts):
                    ghost_pos = state.getGhostPosition(j + 1)
                    dist = util.manhattanDistance((px, py), ghost_pos)
                    if dist < 2.0:
                        safe = False
                        legal.remove(move)
                        break
                if not safe and i < len(legal):
                    move = legal[i]
                if len(legal) == 0:
                    move = move_copy
